...
namespace SlimBus
{
    public class SlimBus : ISlimBus
    {
        ...
        internal SlimBus(IServiceRegistry serviceRegistry, ISerializationProvider serializationProvider, IRoutingDirectionProvider routingDirectionProvider, ICorrelationIdProvider correlationIdProvider, ITransport transport)
        {
        ...
        internal unsafe Task HandleMessage(string identity, ReadOnlyMemory<byte> data, object sender)
        {
            ...
            if (messageModel.Behavior == MessageBehavior.Request)
            {
                if (!_messageSubscriptions.TryGetValue(messageType, out var messageHandler))
                    throw new Exception(); // TODO: Exception types
                var handlerPtr = (delegate*<SlimBus, Delegate, ReadOnlyMemory<byte>, object, Task>)_messageHandlerFuncs[messageType];
            ...
            else if (messageModel.Behavior == MessageBehavior.Command)
            {
                if (!_messageSubscriptions.TryGetValue(messageType, out var messageHandler))
                    throw new Exception(); // TODO: Exception types
                var handlerPtr = (delegate*<SlimBus, Delegate, ReadOnlyMemory<byte>, Task>)_messageHandlerFuncs[messageType];
        ...
        public unsafe void Subscribe<TRequest, TResponse>(OnRequestDelegate<TRequest, TResponse> onRequest)
        {
            ...
            if (!_messageSubscriptions.TryAdd(requestType, onRequest))
                throw new Exception();
            delegate*<SlimBus, Delegate, ReadOnlyMemory<byte>, object, Task> handlerPtr = &HandleIncomingRequest<TRequest, TResponse>;
        ...
        public unsafe void Subscribe<TMessage>(OnMessageDelegate<TMessage> onMessage)
        {
            ...
            delegate*<SlimBus, Delegate, ReadOnlyMemory<byte>, Task> handlerPtr = &HandleIncomingMessage<TMessage>;
        ...
        public void Bind(object obj, IServiceContract serviceContract)
        {
            ...
            foreach (var interfaceModel in serviceContract.Interfaces)
            {
                ...
                {
                    var bindRequestMethod = typeof(SlimBus).GetMethod("BindObjectRequestHandler").MakeGenericMethod(new[] { requestModel.RequestType, requestModel.ResponseType });
                ...
                {
                    var bindCommandMethod = typeof(SlimBus).GetMethod("BindObjectCommandHandler").MakeGenericMethod(new[] { commandModel.CommandType });
        ...
        private static async Task HandleIncomingRequest<TRequest, TResponse>(SlimBus slimBus, Delegate messageHandler, ReadOnlyMemory<byte> data, object sender)
        {
            var deserialized = slimBus._serializationProvider.Deserialize<TRequest>(data);
            var requestType = typeof(TRequest);
            var resp = await ((OnRequestDelegate<TRequest, TResponse>)messageHandler)(deserialized);
            var responseModel = slimBus._serviceRegistry.GetResponseModel(requestType);
            slimBus._serializationProvider.Serialize(resp, (Transport: slimBus._transport, Sender: sender), (data, state) =>
            {
                state.Transport.Publish(state.Sender, data);
            });
        }

        private static Task HandleIncomingMessage<TCommand>(SlimBus slimBus, Delegate messageHandler, ReadOnlyMemory<byte> data)
        {
            var deserialized = slimBus._serializationProvider.Deserialize<TCommand>(data);
